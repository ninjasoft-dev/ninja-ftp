...
MARK_PATH = RESOURCE_DIR / "ninjasoft-mark.png"
...
def _square_mark(size: int) -> Image.Image:
    """Centraliza a marca em uma tela quadrada transparente."""
...
def generate_application_icons() -> None:
    """Atualiza os ícones da janela, da barra de tarefas e dos pacotes Linux."""
    master = _square_mark(512)
    for size in (16, 32, 48, 480):
        target = RESOURCE_DIR / f"{size}x{size}" / "filezilla.png"
        target.parent.mkdir(parents=True, exist_ok=True)
        master.resize((size, size), Image.Resampling.LANCZOS).save(target)

    ico_sizes = [(16, 16), (24, 24), (32, 32), (48, 48), (64, 64), (128, 128), (256, 256)]
    master.save(RESOURCE_DIR / "NinjaTransfer.ico", format="ICO", sizes=ico_sizes)

    encoded_mark = base64.b64encode(MARK_PATH.read_bytes()).decode("ascii")
    svg = (
        '<?xml version="1.0" encoding="UTF-8"?>\n'
        '<svg xmlns="http://www.w3.org/2000/svg" '
        'xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512">\n'
        '  <image x="26" y="26" width="460" height="460" preserveAspectRatio="xMidYMid meet" '
...
def _draw_folder(draw: ImageDraw.ImageDraw, box: tuple[int, int, int, int], colour: tuple[int, ...]) -> None:
    """Desenha uma pasta simples sem preenchimento claro."""
    left, top, right, bottom = box
    tab_width = (right - left) // 3
    draw.line((left, top + 30, left, top, left + tab_width, top, left + tab_width + 24, top + 30), fill=colour, width=28, joint="curve")
    draw.rounded_rectangle((left, top + 30, right, bottom), radius=24, outline=colour, width=28)
...
def generate_toolbar_icons() -> None:
    ...
    for y, colour in ((82, ACCENT), (202, BLUE), (322, SUCCESS)):
        draw.rounded_rectangle((70, y, 410, y + 82), radius=22, outline=colour, width=28)
    ...
    for y in (110, 230, 350):
        draw.rounded_rectangle((52, y - 35, 310, y + 35), radius=20, outline=MUTED, width=26)
...
def main() -> None:
    ...
    if not MARK_PATH.exists():
        raise FileNotFoundError(f"Marca não encontrada: {MARK_PATH}")
